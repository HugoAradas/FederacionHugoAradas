...
// No he terminado el ejercicio 6, pero juro que se hacerlo, en cuanto lo acabe en casa lo subire al repositorio, aunque no puntue, pero quiero terminarlo :)
			
			do {
				System.out.println("Ha seleccionado Ver Medallas.");
				System.out.println("Seleccione 1 para el tipo de metal, 2 para el % de pureza,\n3 para la fecha en la que fue entregado o 4 para si fue asignada o no.");
				subelecc = in.nextInt();
				if (subelecc != 1 && subelecc != 2 && subelecc != 3 && subelecc != 4)
					System.out.println("¡Valor seleccionado no válido!\n");
				else
					valido = true;
			} while (!valido);
			if (subelecc == 1) {
				do {
					System.out.println("Ha seleccionado filtrar por metal");
					System.out.println("Seleccione 1 para ORO, 2 para PLATA o 3 para BRONCE.");
					subelecc = in.nextInt();
					if (subelecc != 1 && subelecc != 2 && subelecc != 3)
						System.out.println("¡Valor seleccionado no válido!\n");
				...
				if (subelecc == 1) {
					System.out.println("Ha seleccionado ORO");
					System.out.println(Datos.OROS);
				} else if (subelecc == 2) {
					System.out.println("Ha seleccionado PLATA");
					System.out.println(Datos.PLATAS);
				} else {
					System.out.println("Ha seleccionado BRONCE");
					System.out.println(Datos.BRONCES);
				}
			} else if (subelecc == 2) {
				do {
					System.out.println("Ha seleccionado filtrar por % de pureza");
					
				} while (!valido);	
			} else if (subelecc ==3) {
				do {
					System.out.println("Ha seleccionado filtrar por fecha");
				} while (!valido);	
			} else {
				do {
					System.out.println("Ha seleccionado filtrar por asignacion");
				} while (!valido);	
			}
			break;
		default:
	...
	private static void mostrarSubmenuAtleta(int elecc) {
		...
		case 1: ////opción 3.1
			...
			Atleta atleta1 = new Atleta(subelecc, subelecc, subelecc, null);
